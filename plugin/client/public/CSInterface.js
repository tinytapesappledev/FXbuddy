...
/**
 * ============================================================================
 * FXbuddy Browser Fallback
 * ============================================================================
 * When running outside Adobe CEP (browser dev mode), window.__adobe_cep__
 * does not exist. The official CSInterface above will throw errors when
 * its methods are called. This section patches CSInterface with safe
 * browser-mode stubs so that development and testing can happen in a browser.
 */
(function() {
    if (typeof window !== 'undefined' && !window.__adobe_cep__) {
        console.log('[CSInterface] No __adobe_cep__ detected — applying browser fallback stubs.');

        // Override prototype methods that reference __adobe_cep__
        ...
        CSInterface.prototype.evalScript = function(script, callback) {
            console.log('[CSInterface] evalScript (browser fallback):', script);
            if (callback) {
                setTimeout(function() {
        ...
        CSInterface.prototype.getSystemPath = function(pathType) {
            console.log('[CSInterface] getSystemPath (browser fallback):', pathType);
            return '/tmp/fxbuddy';
        };

        CSInterface.prototype.addEventListener = function(type, listener, obj) {
            console.log('[CSInterface] addEventListener (browser fallback):', type);
        };

        CSInterface.prototype.removeEventListener = function(type, listener, obj) {
            console.log('[CSInterface] removeEventListener (browser fallback):', type);
        };

        CSInterface.prototype.requestOpenExtension = function(extensionId, params) {
            console.log('[CSInterface] requestOpenExtension (browser fallback):', extensionId);
        };

        CSInterface.prototype.getExtensionID = function() {
            return 'com.fxbuddy.panel';
        };

        CSInterface.prototype.closeExtension = function() {
            console.log('[CSInterface] closeExtension (browser fallback)');
        };
        ...
        CSInterface.prototype.getOSInformation = function() {
            return navigator.platform.indexOf('Mac') >= 0 ? 'Mac OS X' : 'Windows';
        };
        ...
        CSInterface.prototype.dispatchEvent = function(event) {
            console.log('[CSInterface] dispatchEvent (browser fallback):', event.type);
        };
